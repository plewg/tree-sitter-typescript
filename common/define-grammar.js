...
module.exports = function defineGrammar(dialect) {
    return grammar(JavaScript, {
        ...
        externals: ($, previous) =>
            previous.concat([$._function_signature_automatic_semicolon]),
        ...
        inline: ($, previous) =>
            previous
                .filter(
                    (rule) =>
                        !["_formal_parameter", "_call_signature"].includes(
                            rule.name,
                        ),
                )
        ...
        rules: {
                ...
                if (dialect === "typescript") {
                    ...
                    choices.push(
                        ...previous.members.filter(
                            (member) => member.name !== "_jsx_element",
                        ),
                    );
                } else if (dialect === "tsx") {
            ...
            as_expression: ($) =>
                prec.left(
                    "binary",
                    seq($.expression, "as", choice("const", $._type)),
                ),

            satisfies_expression: ($) =>
                prec.left("binary", seq($.expression, "satisfies", $._type)),
            ...
            extends_clause: ($) =>
                seq("extends", commaSep1($._extends_clause_single)),
            ...
            template_literal_type: ($) =>
                seq(
                    "`",
                    repeat(choice($._template_chars, $.template_type)),
                    "`",
                ),
            ...
            _type_query_subscript_expression: ($) =>
                seq(
                    ...
                    field(
                        "index",
                        choice($.predefined_type, $.string, $.number),
                    ),
                    "]",
                ),
            _type_query_call_expression: ($) =>
            ...
            intersection_type: ($) =>
                prec.left(seq(optional($._type), "&", $._type)),
            ...
            function_type: ($) =>
                prec.left(
                    seq(
                        ...
                        field(
                            "return_type",
                            choice($._type, $.asserts, $.type_predicate),
                        ),
                    ),
                ),
